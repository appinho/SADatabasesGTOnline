...
mysqli_select_db($connect, "gtonline_complete") or die("Unable to select database");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	/* FIX this section - retrieve the hometown field from the
		...
		$_POST array and escape it with mysqli_real_escape_string() */
	...
	if (!mysqli_query($connect, $query)) {
		...
		print '<p class="error">Error: Failed to update user profile. ' . mysqli_error() . '</p>';
	...
	if (!empty($_POST['add_interest']) and $_POST['add_interest'] != '(add interest)' and trim($_POST['add_interest']) != '') {
		...
		if (!mysqli_query($connect, $query)) {
			...
			print '<p class="error">Error: Failed to add interest. ' . mysqli_error() . '</p>';
...
if (!empty($_GET['delete_interest'])) {
	...
	if (!mysqli_query($connect, $query)) {
		...
		print '<p class="error">Error: Failed to delete interest. ' . mysqli_error() . '</p>';
...
$query = "SELECT firstname, lastname, sex, birthdate, currentcity " .
